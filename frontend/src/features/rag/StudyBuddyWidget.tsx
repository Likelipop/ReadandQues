...
import { X, Send, Bot, Sparkles, BookOpen, AlertCircle } from 'lucide-react';
...
export const StudyBuddyWidget: React.FC<{ activeArticleId?: string }> = ({ activeArticleId }) => {
  ...
  const handleSend = async () => {
    ...
    // Trigger streaming hook with onComplete callback to persist streamed message
    await startStream(userQ, activeArticleId, (finalText, finalCitations) => {
      if (finalText.trim()) {
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: finalText, citations: finalCitations },
        ]);
      }
    });
  };
    ...
    <div className="fixed bottom-6 right-6 z-50">
      {!isOpen ? (
        ...
        <div
          ...
          className="w-[380px] h-[520px] glass-card glow-violet flex flex-col overflow-hidden animate-in slide-in-from-bottom-6 duration-300"
        >
          {/* Header */}
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3.5 text-xs">
            ...
            {isStreaming && (
              <div className="flex gap-2.5 justify-start" aria-live="polite">
