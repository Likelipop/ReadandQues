...
def trigger_async_exam_generation(article_id: str, original_text: str, title: str, url: str) -> bool:
    """
    ...
    try:
        ...
        logger.info("🚀 Dispatched Celery task generate_exam_task for article %s", article_id)
        return True
    except Exception as exc:
        logger.error("❌ Failed to dispatch Celery task for article %s: %s", article_id, exc)
        return False
