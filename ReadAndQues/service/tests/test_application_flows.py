...
class SingleArticleWorkflowCharacterizationTests(TestCase):
    def build_module(self):
        ...
        dependencies = {
            "service.orchestration.jobs.clean_logic": dependency(
                "service.orchestration.jobs.clean_logic",
                clean_and_validate_article=self.clean,
            ),
            "service.orchestration.jobs.processing": dependency(
                "service.orchestration.jobs.processing",
                clean_and_validate_article=self.clean,
            ),
            ...
            "service.orchestration.jobs.generate_questions": dependency(
                "service.orchestration.jobs.generate_questions", run_ai_pipeline=self.run_ai
            ),
            "service.orchestration.jobs.ai_services": dependency(
                "service.orchestration.jobs.ai_services", run_ai_pipeline=self.run_ai
            ),
        }
        return load_legacy_module(
            "legacy_single_article_for_test",
            "orchestration/jobs/single_article.py",
            dependencies,
        )
...
class OrchestratorCharacterizationTests(TestCase):
    def build_module(self):
        ...
        dependencies = {
            ...
            "service.orchestration.configuration": dependency(
                "service.orchestration.configuration", get_pipe=self.get_pipe
            ),
            "service.orchestration.pipes": dependency("service.orchestration.pipes"),
        }
        return load_legacy_module(
...
class SmartParaphraseCharacterizationTests(TestCase):
    def build_module(self):
        ...
        dependencies = {
            "service.orchestration.configuration": dependency(
                "service.orchestration.configuration", job=decorator
            )
        }
        return load_legacy_module(
            "legacy_paraphrase_logic_for_test",
            "orchestration/jobs/paraphrase_logic.py",
            dependencies,
        )
    ...
    def test_cache_miss_generates_and_preserves_source_offsets(self):
        ...
        ai_module = dependency(
            "service.ai_core.graphs.smart_paraphrase",
            run_smart_paraphrase_llm=run_llm,
        )

        with patch.dict(
            "sys.modules", {"service.ai_core.graphs.smart_paraphrase": ai_module}
        ):
            result = module.logic_smart_paraphrase_llm(
                {},
                "hard",
