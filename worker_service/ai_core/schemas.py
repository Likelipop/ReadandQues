...
class CoreAnalysis(BaseModel):
    ...
    secondary_themes: List[str] = Field(
        default_factory=list, description="Supporting themes, if any"
    )
    tone: str = Field(
        default="", description="e.g. formal, satirical, melancholic, objective…"
    )
    ...
    emotional_arc: Optional[str] = Field(
        default=None, description="Emotional progression across the text, if present"
    )
...
class NarrativeAnalysis(BaseModel):
    ...
    symbolism: List[str] = Field(
        default_factory=list, description="Symbols and their meanings"
    )
    irony_or_foreshadowing: List[str] = Field(default_factory=list)
    narrative_perspective: str = Field(
        default="", description="Point of view, narrator reliability"
    )
    unstated_implications: List[str] = Field(
        default_factory=list, description="Things implied but never stated outright"
    )
...
class PoetryAnalysis(BaseModel):
    ...
    sound_devices: List[str] = Field(
        default_factory=list, description="Rhyme, rhythm, alliteration…"
    )
    form_structure: str = Field(
        default="", description="Poetic form, stanza count, rhyme scheme"
    )
    multiple_interpretations: List[str] = Field(
        default_factory=list, description="Different valid readings of the poem"
    )
...
class PersuasiveAnalysis(BaseModel):
    ...
    rhetorical_strategies: List[str] = Field(
        default_factory=list, description="e.g. emotional appeal, statistical evidence…"
    )
    ...
    bias_indicators: List[str] = Field(
        default_factory=list, description="Signs of bias in the presentation"
    )
...
class SemanticAnalysis(BaseModel):
    genre: TextGenre
    theme: ThemeCategory = Field(
        default=ThemeCategory.general,
        description="Primary theme/topic of the reading passage",
    )
    core: CoreAnalysis
